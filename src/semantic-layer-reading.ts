...
let progressState: ProgressState = loadProgress();
...
const semanticRoles: Record<LayerNumber, SemanticRole[]> = {
  1: [
...
const paraphraseReference: Record<LayerNumber, ReferenceItem[]> = {
  1: [
...
const practiceQuestions: Record<LayerNumber, PracticeQuestion> = {
  1: {
    id: "core-svo",
    prompt: "Câu lõi đúng nhất là gì?",
    options: ["The documents began to define the core components.", "The 14th Congress was relatively comprehensive.", "Vietnam developed strategic breakthroughs."],
    correctIndex: 0,
    success: "Đúng. Bạn đã bắt được Subject + Verb + Object.",
    error: "Chưa đúng. Hãy bỏ tạm các cụm from/in/of/to để tìm xương sống."
  },
  2: {
    id: "modifier-manner",
    prompt: "Cụm “in relatively comprehensive terms” bổ nghĩa cho phần nào?",
    options: ["The documents", "began to define", "the 14th National Party Congress"],
    correctIndex: 1,
    success: "Đúng. Cụm này nói cách/mức độ của hành động “define”.",
    error: "Chưa đúng. Cụm này nằm chen giữa động từ và tân ngữ để bổ nghĩa cho hành động."
  },
  3: {
    id: "head-noun",
    prompt: "Trong cụm “the core components of Vietnam’s development model”, danh từ trung tâm là gì?",
    options: ["Vietnam", "development model", "components"],
    correctIndex: 2,
    success: "Đúng. “components” là hạt nhân, các phần sau chỉ bổ nghĩa.",
    error: "Chưa đúng. Hãy tìm danh từ chính trước rồi đọc các cụm of/in đi sau."
  },
  4: {
    id: "range-logic",
    prompt: "Cấu trúc “from... to...” trong câu này chủ yếu biểu thị điều gì?",
    options: ["Nguyên nhân", "Phạm vi bao quát", "Sự đối lập"],
    correctIndex: 1,
    success: "Đúng. Nó cho thấy phạm vi từ nền tảng đến hành động chiến lược.",
    error: "Chưa đúng. “from... to...” ở đây không phải nguyên nhân hay đối lập."
  },
  5: {
    id: "policy-term",
    prompt: "Trong văn cảnh này, “documents” nên hiểu tự nhiên nhất là gì?",
    options: ["giấy tờ", "văn kiện", "bài tập"],
    correctIndex: 1,
    success: "Đúng. Ngữ cảnh chính trị nên dịch là “văn kiện”.",
    error: "Chưa đúng. Trong văn bản chính trị, “documents” thường là “văn kiện”."
  },
  6: {
    id: "summary-transfer",
    prompt: "Bản tóm tắt đúng nhất của cả câu là gì?",
    options: ["Văn kiện Đại hội XIV bước đầu định hình mô hình phát triển mới của Việt Nam.", "Đại hội XIV chỉ nói về nhiệm vụ trọng tâm.", "Việt Nam đã hoàn thành toàn bộ mô hình phát triển."],
    correctIndex: 0,
    success: "Đúng. Bạn đã chuyển được câu dài thành ý chính ngắn.",
    error: "Chưa đúng. Câu gốc nói “bước đầu định hình”, không nói đã hoàn thành."
  }
};

const transferTasks: Record<LayerNumber, SemanticRole[]> = {
  1: [
    { label: "Đọc → Tóm tắt", value: "The documents began to define the core components." },
    { label: "Đọc → Nói", value: "Câu này nói về việc các văn kiện bắt đầu định hình các thành tố chính." }
  ],
  2: [
    { label: "Đọc → Dịch", value: "in relatively comprehensive terms → một cách tương đối toàn diện" },
    { label: "Đọc → Viết", value: "The documents define the model in a fairly comprehensive way." }
  ],
  3: [
    { label: "Đọc → Tách cụm", value: "components → core components → of Vietnam’s development model" },
    { label: "Đọc → Viết lại", value: "the main elements of Vietnam’s model of development" }
  ],
  4: [
    { label: "Đọc → Logic", value: "Nguồn định hướng → định hình → phạm vi nội dung" },
    { label: "Đọc → Nói", value: "Nó bao quát từ bối cảnh, mục tiêu đến nhiệm vụ và đột phá." }
  ],
  5: [
    { label: "Đọc → Từ vựng ngành", value: "perspectives = quan điểm chỉ đạo; mechanisms = cơ chế vận hành" },
    { label: "Đọc → Dịch chính sách", value: "strategic breakthroughs → đột phá chiến lược" }
  ],
  6: [
    { label: "Đọc → Dịch", value: "Tạo bản dịch chính xác, trang trọng." },
    { label: "Đọc → Tóm tắt", value: "Rút câu dài thành 1 ý chính." },
    { label: "Đọc → Nói", value: "Diễn giải lại bằng tiếng Việt tự nhiên." },
    { label: "Đọc → Viết", value: "Paraphrase lại bằng tiếng Anh đơn giản hơn." }
  ]
};
...
function loadProgress(): ProgressState {
  try {
    ...
    if (!raw) return { xp: 0, solved: 0, streak: 0, answered: {} };
...
function toAppMode(value: string | null): AppMode {
  return value === "practice" ? "practice" : "learn";
}

function renderSemanticRoles(layer: LayerNumber): string {
...
function renderParaphraseLadder(layer: LayerNumber): string {
...
function renderTransferTasks(layer: LayerNumber): string {
...
function renderLearnPanel(layer: LayerNumber): string {
...
function renderPracticePanel(layer: LayerNumber): string {
...
function setActiveLayerButton(layer: LayerNumber): void {
...
function setActiveModeButton(mode: AppMode): void {
...
function updateProgressUi(): void {
  ...
  if (xpValue) xpValue.textContent = String(progressState.xp);
  if (solvedValue) solvedValue.textContent = String(progressState.solved);
  if (streakValue) streakValue.textContent = String(progressState.streak);
  if (progressFill) progressFill.style.width = `${Math.min(100, progressState.solved / 6 * 100)}%`;
  if (microGoal) microGoal.textContent = progressState.solved >= 6 ? "Hoàn thành vòng luyện đầu tiên. Có thể luyện lại để tăng tốc đọc." : "Mục tiêu nhỏ: trả lời đúng 1 câu ở layer hiện tại.";
}
...
function handlePracticeAnswer(button: HTMLButtonElement): void {
  const question = practiceQuestions[currentLayer];
  const selectedIndex = Number(button.dataset.answer);
  const isCorrect = selectedIndex === question.correctIndex;
  const feedback = document.getElementById("practiceFeedback");

  document.querySelectorAll<HTMLButtonElement>("[data-answer]").forEach((optionButton) => {
...
function bindModeButtons(): void {
  document.querySelectorAll<HTMLButtonElement>("[data-mode]").forEach((button) => {
    button.addEventListener("click", () => setMode(toAppMode(button.dataset.mode ?? null)));
  });
}

function bindPracticeDelegation(): void {
