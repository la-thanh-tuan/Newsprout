...
function renderPatternRows<T extends string>(items: LayerPattern<T>[]): string {
  return items
    .map((item) => `
      <tr>
...
function renderCatalogTable<T extends string>(title: string, description: string, items: LayerPattern<T>[]): string {
  return `
    <div class="box">
      <h3>${title}</h3>
      <p>${description}</p>
...
function renderSentenceAnalysis(sentence: string, result: DetectedLayerSet, index: number): string {
  return `
...
function analyzeInputText(): void {
  const input = document.getElementById("analysisInput") as HTMLTextAreaElement | null;
  ...
  output.innerHTML = sentences
    .map((sentence, index) => renderSentenceAnalysis(sentence, detectSentenceLayers(sentence), index))
    .join("");
}

function bindAnalysisControls(): void {
